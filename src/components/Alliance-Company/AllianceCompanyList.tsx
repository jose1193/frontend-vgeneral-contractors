...
interface AllianceCompanyListProps {
  allianceCompanies: AllianceCompanyData[];
  onDelete: (uuid: string) => void;
  userRole: string | undefined;
}

const AllianceCompanyList: React.FC<AllianceCompanyListProps> = ({
  allianceCompanies,
  onDelete,
  userRole,
}) => {
  ...
  const handleDeleteConfirm = async () => {
    if (companyToDelete?.uuid) {
      ...
      try {
        ...
        setSnackbar({
          open: true,
          message: "Failed to delete alliance company",
          severity: "error",
        });
      } finally {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        ...
        >
          <Link
            href={`/dashboard/alliance-companies/${params.row.uuid}`}
            passHref
          >
            <Button
              size="small"
          ...
          <Link
            href={`/dashboard/alliance-companies/${params.row.uuid}/edit`}
            passHref
          >
            <Button
              size="small"
  ...
  const rows = allianceCompanies.map((company) => ({
    id: company.uuid,
    uuid: company.uuid,
    name: company.alliance_company_name,
    email: company.email,
    phone: company.phone,
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Are you sure you want to delete this alliance company?
          </Typography>
          <Typography
            variant="body1"
            gutterBottom
          ...
          >
            ...
            <span style={{ fontWeight: "bold", marginLeft: 10 }}>
              {companyToDelete?.alliance_company_name}
            </span>
          </Typography>
          <Typography
            variant="body1"
...
export default AllianceCompanyList;
