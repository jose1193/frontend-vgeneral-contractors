...
interface ClaimAgreementAllianceListProps {
...
const ClaimAgreementAllianceList: React.FC<ClaimAgreementAllianceListProps> = ({
  items,
  onDelete,
  onRestore,
  userRole,
  ...
  const [selectedItem, setSelectedItem] =
    useState<ClaimAgreementAllianceData | null>(null);
  ...
  const handleEdit = (uuid: string) => {
    router.push(`/dashboard/claim-agreement-alliances/${uuid}/edit`);
  };

  const handleDeleteClick = (item: ClaimAgreementAllianceData) => {
    setSelectedItem(item);
    setDeleteDialogOpen(true);
  };

  const handleRestoreClick = (item: ClaimAgreementAllianceData) => {
    setSelectedItem(item);
  ...
  const handleDeleteConfirm = async () => {
    if (selectedItem?.uuid) {
      ...
      try {
        ...
        setSnackbar({
          open: true,
          message: "Claim Agreement Alliance suspended successfully",
          severity: "success",
        });
      } catch (error) {
        setSnackbar({
          open: true,
          message: "Failed to suspend Claim Agreement Alliance",
          severity: "error",
        });
      } finally {
  ...
  const handleRestoreConfirm = async () => {
    if (selectedItem?.uuid) {
      ...
      try {
        ...
        setSnackbar({
          open: true,
          message: "Claim Agreement Alliance restored successfully",
          severity: "success",
        });
      } catch (error) {
        setSnackbar({
          open: true,
          message: "Failed to restore Claim Agreement Alliance",
          severity: "error",
        });
      } finally {
        setIsSubmitting(false);
  ...
  const columns: GridColDef[] = [
    {
      field: "alliance_company",
      headerName: "Alliance Company",
      flex: 1,
      minWidth: 150,
      headerAlign: "center",
      align: "center",
    ...
    },
    {
      ...
      getActions: (params) => [
        ...
        <GridActionsCellItem
          ...
          showInMenu
        />,
        ...(params.row.status === "Available" && userRole !== "Salesperson"
          ? [
              <GridActionsCellItem
                key="delete"
    ...
    >
          ...
          >
            <DataGrid
              rows={items.map((item) => ({
                ...
                alliance_company:
                  item.alliance_company?.alliance_company_name || "-",
                alliance_company_id: item.alliance_company_id || "-",
                claim: item.claim || "-",
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Are you sure you want to suspend this alliance agreement?
          </Typography>
          {selectedItem && (
            <Box>
              <Typography variant="body1" gutterBottom sx={{ mb: 2 }}>
                ...
                <span style={{ fontWeight: "bold", marginLeft: 10 }}>
                  {typeof selectedItem.alliance_company === "string"
                    ? selectedItem.alliance_company
                    : selectedItem.alliance_company?.alliance_company_name ||
                      "-"}
                </span>
              </Typography>
              <Typography variant="body1" gutterBottom sx={{ mb: 2 }}>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            Are you sure you want to restore this alliance agreement?
          </Typography>
          {selectedItem && (
            <Box>
              <Typography variant="body1" gutterBottom sx={{ mb: 2 }}>
...
export default ClaimAgreementAllianceList;
