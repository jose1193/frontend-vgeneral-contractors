...
interface ClaimAgreementListProps {
  items: ClaimAgreementData[];
  onDelete: (uuid: string) => Promise<ClaimAgreementData | void>;
  onRestore: (uuid: string) => Promise<ClaimAgreementData | void>;
  userRole?: string;
}

const ClaimAgreementList: React.FC<ClaimAgreementListProps> = ({
  items,
  onDelete,
  onRestore,
  userRole,
  ...
  const [selectedItem, setSelectedItem] = useState<ClaimAgreementData | null>(
    null
  );
  ...
  const handleDeleteClick = (item: ClaimAgreementData) => {
    setSelectedItem(item);
    setDeleteDialogOpen(true);
  };

  const handleRestoreClick = (item: ClaimAgreementData) => {
    setSelectedItem(item);
  ...
  const handleDeleteConfirm = async () => {
    if (selectedItem?.uuid) {
      ...
      try {
        ...
        setSnackbar({
          open: true,
          message: "Claim Agreement suspended successfully",
          severity: "success",
        });
      } catch (error) {
        setSnackbar({
          open: true,
          message: "Failed to suspend Claim Agreement",
          severity: "error",
        });
      } finally {
  ...
  const handleRestoreConfirm = async () => {
    if (selectedItem?.uuid) {
      ...
      try {
        ...
        setSnackbar({
          open: true,
          message: "Claim Agreement restored successfully",
          severity: "success",
        });
      } catch (error) {
        setSnackbar({
          open: true,
          message: "Failed to restore Claim Agreement",
          severity: "error",
        });
      } finally {
        setIsSubmitting(false);
...
export default ClaimAgreementList;
